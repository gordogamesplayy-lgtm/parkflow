...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderDashboard() {
        ...
        elements.capLabelRoberto.innerHTML = `<span>Vagas: ${robertoCount} / ${configs.capRoberto}</span>`;
        elements.capLabelTerreno.innerHTML = `<span>Vagas: ${terrenoCount} / ${configs.capTerreno}</span>`;
        elements.capLabelBarracao.innerHTML = `<span>Vagas: ${barracaoCount} / ${configs.capBarracao}</span>`;
    ...
    function renderActiveTable() {
                ...
                <tr class="empty-state-row">
                    <td colspan="6">
                        <div class="empty-state">
                            ...
                            <p>${query ? 'Nenhum veículo correspondente encontrado' : 'Nenhum veículo alocado neste pátio'}</p>
                        </div>
                    </td>
                </tr>
            `;
        ...
        elements.tbodyActive.innerHTML = filtered.map(car => {
            ...
            return `
                <tr>
                    <td><span class="badge-plate ${isMercosul ? 'badge-plate-mercosul' : ''}">${car.plate}</span></td>
                    <td style="font-weight:600;">${capitalizeWords(car.model)}</td>
                    <td><span class="badge-yard ${getYardClass(car.yard)}">${getYardNameDisplay(car.yard)}</span></td>
                    <td>${formattedEntry}</td>
                    <td style="font-size:0.88rem;color:var(--text-secondary);">${car.bank}</td>
                    <td class="text-right">
    ...
    function renderHistory() {
                ...
                <tr class="empty-state-row">
                    <td colspan="7">
                        <div class="empty-state">
                            ...
                            <p>${query ? 'Nenhum registro encontrado no histórico' : 'Nenhuma saída registrada no histórico'}</p>
                        </div>
                    </td>
                </tr>
            `;
        ...
        elements.tbodyHistory.innerHTML = filtered.map(item => {
            ...
            return `
                <tr>
                    <td><span class="badge-plate ${isMercosul ? 'badge-plate-mercosul' : ''}">${item.plate}</span></td>
                    <td style="font-weight:600;">${capitalizeWords(item.model)}</td>
                    ...
                    <td style="font-size:0.88rem;color:var(--text-secondary);">${item.bank}</td>
    ...
    function populateBankDropdown() {
        ...
        elements.selectBanco.innerHTML = `
            ...
            ` + registeredBanks.map(bank => `<option value="${bank}">${bank}</option>`).join('');
    }
    ...
    function renderLeilaoTable() {
                ...
                <tr class="empty-state-row">
                    <td colspan="13">
                        <div class="empty-state">
                            ...
                            <p>${query ? 'Nenhum registro correspondente no leilão' : 'Nenhum veículo enviado para o leilão ainda'}</p>
                        </div>
                    </td>
                </tr>
            `;
        ...
        elements.tbodyLeilao.innerHTML = filtered.map(item => {
            ...
            return `
                <tr>
                    <td><span class="badge-plate ${isMercosul ? 'badge-plate-mercosul' : ''}">${item.plate}</span></td>
                    <td style="font-weight:600; color: var(--text-primary);">${capitalizeWords(item.model)}</td>
                ...
                <tr class="empty-state-row">
                    <td colspan="${colspanVal}">
                        <div class="empty-state">
                            ...
                            <p>${query || selectedMonthKey !== 'all' ? 'Nenhum registro correspondente nos embarcados' : 'Nenhum veículo embarcado para leilão ainda'}</p>
                        </div>
                    </td>
                </tr>
            `;
    ...
    function processCsvImport(text) {
        ...
        for (let i = 1; i < lines.length; i++) {
            const rowFields = lines[i].split(';').map(field => field.replace(/^["']|["']$/g, '').trim());
